...
function getSuggestion(user) {
    var deferred = Q.defer();
    var sql = "SELECT u.id AS id_user, u.first_name, u.last_name,u.city, (ABS(ROUND("+db.connection.escape(user.lng)+", 2) - ROUND(u.lng, 2)) + ABS(ROUND("+db.connection.escape(user.lat)+", 2) - ROUND(u.lat, 2))) AS distance, u.zip, u.lat, u.lng, COUNT(ci.id_interest) as commonInterest, ABS(STR_TO_DATE("+db.connection.escape(user.birth_date)+", '%d/%m/%Y') - STR_TO_DATE(u.birth_date, '%d/%m/%Y')) AS diff_birth, u.birth_date, u.gender, u.orientation, p.photo_link FROM users u LEFT JOIN usersInterests ui ON ui.id_user = u.id LEFT JOIN (SELECT id_interest FROM usersInterests WHERE id_user = "+db.connection.escape(user.id)+") ci ON ci.id_interest = ui.id_interest LEFT JOIN photos p ON p.id_user = u.id AND p.isProfil = 1 WHERE u.id <> "+db.connection.escape(user.id)+" AND u.gender LIKE (CASE "+db.connection.escape(user.gender)+" WHEN 'm' THEN ( CASE "+db.connection.escape(user.orientation)+" WHEN 'Hetero' THEN 'f' WHEN 'Homo' THEN 'm' ELSE '%%' END) WHEN 'f' THEN ( CASE "+db.connection.escape(user.orientation)+" WHEN 'Hetero' THEN 'm' WHEN 'Homo' THEN 'f' ELSE '%%' END) END) AND u.orientation LIKE (CASE "+db.connection.escape(user.orientation)+" WHEN 'Hetero' THEN 'Hetero' WHEN ('Homo' AND u.orientation = 'Homo') THEN 'Homo' WHEN ('Homo' AND u.orientation = 'Bi') THEN 'Bi' WHEN 'Bi' THEN (CASE WHEN u.gender = "+db.connection.escape(user.gender)+" AND u.orientation = 'Bi' THEN 'Bi' WHEN u.gender = "+db.connection.escape(user.gender)+" AND u.orientation = 'Homo' THEN 'Homo' WHEN u.gender <> "+db.connection.escape(user.gender)+" THEN 'Bi' END) END) GROUP BY u.id, ui.id_user, p.id ORDER BY distance ASC, diff_birth ASC, commonInterest DESC"
