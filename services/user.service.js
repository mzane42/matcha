...
function getByIdUser(_id, me) {
    var deferred = Q.defer();
    var sql = 'SELECT users.id, bio, email, last_name, first_name, login, password, birth_date, gender, popularity, users.created_at, users.connected as actif, offline_since, active_since,orientation, GROUP_CONCAT(i.interest_name) as interests, lat, lng, city, zip, country, p.photo_link, (ma.id IS NOT NULL) AS matched, (bk.id IS NOT NULL) AS blocked, (rp.id IS NOT NULL) AS reported, (cn.id IS NOT NULL) AS connected FROM `users` LEFT JOIN usersInterests ui ON ui.id_user = users.id LEFT JOIN interests i On i.id = ui.id_interest LEFT JOIN photos p ON p.id_user = users.id AND p.isProfil = 1 LEFT JOIN matched ma ON (ma.id_author = '+db.connection.escape(me)+' AND ma.id_receiver = users.id) LEFT JOIN matched cn ON ((cn.id_author = '+db.connection.escape(_id)+' AND cn.id_receiver = '+db.connection.escape(me)+') and cn.connected = 1) LEFT JOIN blocked bk ON (bk.id_author = '+db.connection.escape(me)+' AND bk.id_receiver = users.id)  LEFT JOIN reported rp ON (rp.id_author = '+db.connection.escape(me)+' AND rp.id_receiver = users.id) WHERE users.id = ? GROUP BY users.id, ma.id, cn.id, p.id, ui.id_user, rp.id, bk.id'
    db.connection.query(sql, _id, function(err, result) {
        if (err) deferred.reject(err.name + ': ' + err.message);
        if (result) {
            deferred.resolve(_.omit(result[0], 'password'));
